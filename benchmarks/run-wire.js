...
//
// For each workload + size, the Elm worker hands back two JSON payloads — a full
// document (`OpDoc.encode`) and the delta of one more typical edit
// (`OpDoc.encodeSince`) — and we measure raw vs gzipped bytes for each. Deltas are
// the interesting case: a tiny payload gives gzip a cold dictionary, so that's where
// a custom format could plausibly beat gzip(JSON). Full docs are where gzip is
// strongest (lots of repeated keys/replica strings to crush).
//
//   cd benchmarks
